...
- **Landing redesign:** hero now shows a live, DB-backed stats strip (real cafe
  ...
  live-location ping, the featured card, and a peeking second result). The mock's
  "Get directions" bar is a real link into `/app` (hover + focus ring) so it
  enters the app instead of dead-ending; the rest of the card is non-interactive.
